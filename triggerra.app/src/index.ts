...
async function main() {
  ...
  app.get('/health', async () => {
    return { status: 'ok', service: 'triggerra.app' };
  });

  app.get('/', async () => {
    return { message: 'Triggerra App - Workflow Automation System' };
  });

  const metricsPort = parseInt(process.env.TRIGR_APP_METRICS_PORT || String(port + 1), 10);
  await startMetricsServer({ port: metricsPort });
  await app.listen({ port, host: '0.0.0.0' });
  logger.info({ port, service: serviceName }, 'Triggerra App started');
}

main().catch((err) => {
  logger.error(err, 'Fatal error in triggerra.app');
  process.exit(1);
});
