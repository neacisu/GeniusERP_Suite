#!/usr/bin/env bash
# Process Supervisor launcher for CP Suite Admin
set -eu

ENV_FILE="/app/secrets/.env"
APP_NAME="cp-suite-admin"
...
for var in CP_ADMIN_DB_POSTGRES_URL CP_ADMIN_AUTH_JWT_SECRET CP_ADMIN_API_AUDIT_WEBHOOK_URL; do
  if [ -z "${!var:-}" ]; then
