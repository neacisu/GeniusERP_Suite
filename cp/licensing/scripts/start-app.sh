#!/usr/bin/env bash
# Process Supervisor launcher for CP Licensing
set -eu

ENV_FILE="/app/secrets/.env"
APP_NAME="cp-licensing"
APP_DIST_DIR="/app/cp/licensing/dist"
APP_PACKAGE_SUBPATH="cp/licensing"
...
for var in CP_LIC_DB_POSTGRES_URL CP_LIC_AUTH_JWT_SECRET CP_LIC_MASTER_KEY CP_LIC_API_STRIPE_SECRET_KEY CP_LIC_API_STRIPE_WEBHOOK_SECRET; do
  if [ -z "${!var:-}" ]; then
