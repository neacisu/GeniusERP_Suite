#!/usr/bin/env bash
# Process Supervisor launcher for CP AI Hub
set -eu

ENV_FILE="/app/secrets/.env"
APP_NAME="cp-ai-hub"
APP_DIST_DIR="/app/cp/ai-hub/dist"
APP_PACKAGE_SUBPATH="cp/ai-hub"
...
for var in CP_AI_DB_POSTGRES_URL CP_AI_RAG_VECTOR_DB_URL CP_AI_AUTH_JWT_SECRET CP_AI_API_OPENAI_KEY CP_AI_API_ANTHROPIC_KEY CP_AI_API_GOOGLE_KEY; do
  if [ -z "${!var:-}" ]; then
