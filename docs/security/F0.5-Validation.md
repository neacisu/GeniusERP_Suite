...
**Proxy Chaos Prereq**: export `PROXY_APPROLE_ROLE_ID_PATH` and `PROXY_APPROLE_SECRET_ID_PATH` so the proxy compose stack can mount the AppRole material before rerunning the proxy chaos scenario. Example:

```bash
export PROXY_APPROLE_ROLE_ID_PATH=.secrets/approle/proxy/role-id
export PROXY_APPROLE_SECRET_ID_PATH=.secrets/approle/proxy/secret-id
```
...
## Latest Evidence (2025-11-25)

- Command: `scripts/security/test-f05-chaos.sh all | tee -a docs/security/evidence/chaos-suite-2025-11-25.log`
- Evidence log: `docs/security/evidence/chaos-suite-2025-11-25.log`
- Result: PASS for every OpenBao-integrated workload and proxy scenario
