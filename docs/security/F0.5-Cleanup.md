...
## 5. Automation Script & Evidence

Use `scripts/security/github-secrets-cleanup.sh` to enforce the cleanup on every repository clone:

```bash
# Plan-only mode (shows which secrets would be removed)
./scripts/security/github-secrets-cleanup.sh --evidence docs/security/evidence/github-secrets-cleanup-$(date +%F).json

# Execute deletion + capture evidence
./scripts/security/github-secrets-cleanup.sh --delete --evidence docs/security/evidence/github-secrets-cleanup-$(date +%F).json
```

The script relies on the GitHub CLI (`gh`) to list and delete repository-level secrets. The evidence JSON should be checked in under `docs/security/evidence/` together with a short Markdown summary (date, operator, outcome) so audits can link to raw data.

### Evidence Template

Create a file named `docs/security/evidence/github-secrets-cleanup-YYYY-MM-DD.md` with content similar to:

```markdown
# GitHub Secrets Cleanup — YYYY-MM-DD

- Operator: <nume>
- Repository: <org/repo>
- Command: `./scripts/security/github-secrets-cleanup.sh --delete --evidence docs/security/evidence/github-secrets-cleanup-YYYY-MM-DD.json`
- Snapshot: `github-secrets-cleanup-YYYY-MM-DD.json`

| Secret | Status | Notes |
|--------|--------|-------|
| BAO_TOKEN | Deleted | Replaced by OIDC |
| GH_PAT_TOKEN | Deleted | Vault path `kv/data/deployments/github` |
| NPM_TOKEN | Deleted | Vault path `kv/data/deployments/npm` |
| DB_PASSWORD | Deleted | Dynamic credentials | 
| JWT_SECRET | Deleted | Static KV in OpenBao |
| DOCKER_USERNAME / DOCKER_PASSWORD | Deleted | Vault path `kv/data/deployments/docker` |
```

Attach both files to the PR/Change Request that documents the cleanup.
