#!/usr/bin/env bash
#
# setup-approle.sh - Generate AppRole credentials for geniuserp
#
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
SECRETS_DIR="${REPO_ROOT}/.secrets/approle/geniuserp"
...
echo -e "${BLUE}═══════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  geniuserp AppRole Setup${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════${NC}"
echo ""
...
echo -e "${BLUE}[1/3] Creating AppRole for geniuserp...${NC}"

bao write auth/approle/role/geniuserp \
    token_ttl=1h \
    token_max_ttl=4h \
    token_policies="geniuserp-read" \
    bind_secret_id=true \
    secret_id_ttl=0

echo -e "${GREEN}  ✓ AppRole 'geniuserp' created${NC}"
echo ""

# Get role-id
echo -e "${BLUE}[2/3] Retrieving role-id...${NC}"

ROLE_ID=$(bao read -field=role_id auth/approle/role/geniuserp/role-id)
...
SECRET_ID=$(bao write -field=secret_id -f auth/approle/role/geniuserp/secret-id)
...
echo ""
echo -e "${BLUE}Next: docker compose -f geniuserp.app/compose/docker-compose.yml up${NC}"
echo ""
