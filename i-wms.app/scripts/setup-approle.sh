#!/usr/bin/env bash
#
# setup-approle.sh - Generate AppRole credentials for i-wms
#
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
SECRETS_DIR="${REPO_ROOT}/.secrets/approle/i-wms"
...
echo -e "${BLUE}═══════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  i-wms AppRole Setup${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════${NC}"
echo ""
...
echo -e "${BLUE}[1/3] Creating AppRole for i-wms...${NC}"

bao write auth/approle/role/i-wms \
    token_ttl=1h \
    token_max_ttl=4h \
    token_policies="i-wms-read" \
    bind_secret_id=true \
    secret_id_ttl=0

echo -e "${GREEN}  ✓ AppRole 'i-wms' created${NC}"
echo ""

# Get role-id
echo -e "${BLUE}[2/3] Retrieving role-id...${NC}"

ROLE_ID=$(bao read -field=role_id auth/approle/role/i-wms/role-id)
...
SECRET_ID=$(bao write -field=secret_id -f auth/approle/role/i-wms/secret-id)
...
echo ""
echo -e "${BLUE}Next: docker compose -f i-wms.app/compose/docker-compose.yml up${NC}"
echo ""
