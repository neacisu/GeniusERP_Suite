#!/usr/bin/env bash
#
# setup-approle.sh - Generate AppRole credentials for Numeriqo
#
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
SECRETS_DIR="${REPO_ROOT}/.secrets/approle/numeriqo"
...
echo -e "${BLUE}═══════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Numeriqo AppRole Setup${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════${NC}"
echo ""
...
echo -e "${BLUE}[1/3] Creating AppRole for Numeriqo...${NC}"

bao write auth/approle/role/numeriqo \
    token_ttl=1h \
    token_max_ttl=4h \
    token_policies="numeriqo-read" \
    bind_secret_id=true \
    secret_id_ttl=0

echo -e "${GREEN}  ✓ AppRole 'numeriqo' created${NC}"
echo ""

# Get role-id
echo -e "${BLUE}[2/3] Retrieving role-id...${NC}"

ROLE_ID=$(bao read -field=role_id auth/approle/role/numeriqo/role-id)
...
SECRET_ID=$(bao write -field=secret_id -f auth/approle/role/numeriqo/secret-id)
...
echo ""
echo -e "${BLUE}Next: docker compose -f numeriqo.app/compose/docker-compose.yml up${NC}"
echo ""
