...
```JSON
  "F0.5.10": {
    ...
    "status": "completed",
    "note_implementare": "S-a finalizat scriptul automatizat `scripts/security/openbao-enable-db-engine.sh`, acum parametrizat după `.suite.general.env` (host `postgres_server`, port 5432, DB `postgres`) și **fără secrete hardcodate**. Scriptul generează (sau reutilizează din `.secrets/openbao-db-admin.pass`) parola `openbao_admin`, o aplică indiferent dacă rolul există deja, configurează `database/` secrets engine cu `postgresql-database-plugin`, setează TTL-urile implicite (1h/24h), instalează CLI-ul `bao` pe host și generează rolul de probă `example-readonly` pentru validare. Toate operațiile sunt idempotente, curăță utilizatorul temporar creat pentru test și pregătesc terenul pentru F0.5.11 / F0.5.12.",
    "validare_hands_on": "1. `export BAO_ADDR=http://127.0.0.1:8200 BAO_TOKEN=$(jq -r '.root_token' .secrets/openbao-keys.json) && ./scripts/security/openbao-enable-db-engine.sh` → scriptul rulează cap-coadă, generează parola în `.secrets/openbao-db-admin.pass` și afișează sumarul *Database Engine: ACTIVE*, *Default TTL: 1 hour*, *Example Role: example-readonly*. 2. `docker exec -e BAO_TOKEN=… -e BAO_ADDR=http://127.0.0.1:8200 geniuserp-openbao bao secrets list` → include montajul `database/`, confirmând activarea engine-ului. 3. `docker exec -e BAO_TOKEN=… -e BAO_ADDR=http://127.0.0.1:8200 geniuserp-openbao bao read database/creds/example-readonly` → emite utilizator efemer `v-root-example--*` cu `lease_duration=1h`; scriptul șterge automat rolul (verificat prin `REVOKE/DROP ROLE` în Postgres)."
  },
```
...
```JSON
  "F0.5.11": {
    ...
    "status": "completed",
    "note_implementare": "Manifestul `database/roles/roles.json` include acum 14 roluri runtime (stand-alone + CP), fiecare mapat la un template `*_runtime.sql`. Scriptul `scripts/security/openbao-sync-app-roles.sh` creează bazele lipsă, aplică revocation statements și, pentru RAG, instalează extensiile declarate (pgvector). Postgres rulează pe o imagine custom `scripts/postgres/Dockerfile.pgvector` astfel încât `CREATE EXTENSION vector` este disponibil înainte de sincronizarea rolurilor. Documentația `database/roles/README.md` explică și câmpul `extensions` pentru cazurile care au nevoie de module suplimentare.",
    "validare_hands_on": "1. `set -a && source .suite.general.env && set +a && export BAO_ADDR=http://127.0.0.1:8200 BAO_TOKEN=$(jq -r '.root_token' .secrets/openbao-keys.json) && ./scripts/security/openbao-sync-app-roles.sh` → recreează toate conexiunile, creează DB-ul `ai_vectors_db` și activează `pgvector` înainte de a configura `cp_aihub_vectors_runtime`. 2. `bao read -format=json database/creds/cp_aihub_vectors_runtime` → emite utilizator efemer cu `lease_duration=3600`, după care `bao lease revoke <lease_id>` șterge automat rolul din Postgres (confirmat cu `SELECT 1 FROM pg_roles WHERE rolname='<user>'` → fără rezultate). 3. `docker compose up -d --build postgres_server` construiește imaginea pgvector și păstrează volumele existente, validând că rotația rolurilor rămâne funcțională pentru toate aplicațiile."
  },
```
...
```JSON
  "F0.5.14": {
    ...
    "status": "completed",
    "note_implementare": "S-a implementat complet **Process Supervisor pattern** pentru Numeriqo ca aplicație pilot: (1) **Dockerfile actualizat** să folosească imaginea `geniuserp/node-openbao:local` cu multi-stage build păstrând optimizările existente, (2) **OpenBao Agent config** (`numeriqo.app/openbao/agent-config.hcl`) cu auto-auth AppRole și 3 template-uri (db-creds.json pentru credențiale dinamice, app-secrets.json pentru KV static, numeriqo.env.tpl pentru .env combinat), (3) **Process Supervisor script** (`scripts/start-app.sh`) care validează secretele injectate și execută `node dist/...`, (4) **docker-compose.yml** actualizat cu volume mounts pentru AppRole credentials (`role-id`, `secret-id`) și eliminarea dependenței de `.numeriqo.env`, (5) **Helper script** (`scripts/setup-approle.sh`) pentru generarea automată a AppRole credentials, (6) **README complet** cu diagramă arhitectură, instrucțiuni setup, flow de injectare secrete și troubleshooting. Aplicația pornește **exclusiv cu secrete injectate** din OpenBao, fără fallback la `.env` local.",
    "validare_hands_on": "1. `export BAO_TOKEN=$(jq -r '.root_token' .secrets/openbao-keys.json) BAO_ADDR=http://127.0.0.1:8200 && cd numeriqo.app && ./scripts/setup-approle.sh` → generează AppRole credentials în `.secrets/approle/numeriqo/{role-id,secret-id}`. 2. `docker compose -f numeriqo.app/compose/docker-compose.yml up --build` → container pornește, OpenBao Agent se autentifică cu AppRole, renderizează template-urile în `/app/secrets/`, execută `start-app.sh` care afișează `[Process Supervisor] ✓ All secrets validated` și `[Process Supervisor] ✓ Database user: v-root-numeriqo-*`, apoi Node.js app pornește cu succes. 3. `docker exec genius-suite-numeriqo-app ls -la /app/secrets/` confirmă existența fișierelor `.env`, `db-creds.json`, `app-secrets.json`. 4. `curl http://localhost:6750/health` returnează `200 OK`, demonstrând că aplicația rulează cu credențiale dinamice din OpenBao. Validare completă conform cerințelor din JSON - aplicația pornește **fără `.env` local**, doar cu secrete injectate."
  },
```
...
```JSON
  "F0.5.15": {
    ...
    "status": "completed",
    "note_implementare": "S-a creat pachetul complet de **Developer Experience** pentru Process Supervisor pattern: (1) **Ghid complet** (`docs/devx/OpenBao-Process-Supervisor.md`, 600+ linii) cu arhitectură detaliată, quick start, explicații how-it-works (AppRole auth, template rendering, Process Supervisor), opțiuni development (full Process Supervisor vs local .env fallback), scenarii pentru migrații și CLI commands, debugging complet, troubleshooting pentru toate cazurile comune și FAQ extins, (2) **Quick Reference Card** (`docs/devx/OpenBao-Quick-Reference.md`) cu cheat sheet pentru operațiuni zilnice (secrets management, dynamic DB credentials, AppRole, policies), workflow-uri aplicații, troubleshooting rapid și proceduri emergency, (3) **Helper commands** în `package.json` (7 comenzi noi: `openbao:init`, `openbao:login`, `openbao:seed`, `openbao:db-engine`, `openbao:sync-roles`, `dev:openbao` pentru setup complet), (4) **Integrare în start-suite.sh** - adăugat FAZA 2.5 pentru pornire și inițializare automată OpenBao înainte de backing services. Documentația acoperă toate scenariile: pornire aplicații, debugging, CLI/migrații în containere, troubleshooting și emergency procedures.",
    "validare_hands_on": "1. `pnpm dev:openbao` → execută secvențial toate comenzile helper (init, seed, db-engine, sync-roles) și configurează complet OpenBao pentru development. 2. Verificare ghid: `docs/devx/OpenBao-Process-Supervisor.md` conține toate secțiunile cerute (Overview, Quick Start, How It Works, Local Development, Running Migrations, Debugging, Troubleshooting, FAQ) cu exemple concrete și comenzi funcționale. 3. Verificare quick reference: `docs/devx/OpenBao-Quick-Reference.md` oferă cheat sheet complet pentru operațiuni zilnice. 4. `bash scripts/start-suite.sh` → include acum FAZA 2.5 care pornește OpenBao automat și verifică dacă este inițializat, eliminând pași manuali pentru dezvoltatori. 5. Testare workflow complet: un dezvoltator nou poate urma ghidul și să pornească Numeriqo cu Process Supervisor fără ajutor extern. Validare completă conform cerințelor din JSON - DX clar pentru F0.5, reducând workaround-urile per-app."
  },
```
...
```JSON
  "F0.5.16": {
    ...
    "descriere_lunga_si_detaliata_task": "Creează fișiere `.ctmpl` (ex. `numeriqo.app/env/openbao-static.ctmpl`) care mapează secrete KV la variabile NUMQ_* și sunt procesate de agent înainte de a porni aplicația.",
    "directorul_directoarele": [
      "/var/www/GeniusSuite/*/env",
    ...
    "status": "completed",
    "note_implementare": "S-a implementat complet **templating static** pentru toate cele 9 aplicații folosind OpenBao Agent templates (format Go template, nu Consul Template): (1) **Template-uri pentru credențiale DB dinamice** (`openbao/templates/db-creds.tpl`) care extrag username/password din `database/creds/{app}_runtime` și le renderizează în format JSON, (2) **Template-uri pentru secrete statice** (`openbao/templates/app-secrets.tpl`) care extrag jwt_secret, api_key, encryption_key din `kv/data/apps/{app}` și le renderizează în JSON, (3) **Template-uri combinate .env** (`openbao/templates/{app}.env.tpl`) care combină credențiale dinamice DB, secrete statice KV și configurații non-secrete din environment într-un singur fișier `.env` renderizat în `/app/secrets/.env`, (4) **Automatizare completă** prin scripturi (generate-app-config.sh generează automat toate template-urile pentru orice aplicație, auto-create-env-files.py creează fișiere .env.example cu structura corectă), (5) **Eliminare completă valori hardcodate** - toate cele 9 aplicații (numeriqo, archify, mercantiq, flowxify, triggerra, cerniq, i-wms, vettify, geniuserp) au docker-compose.yml fără nicio valoare hardcodată, toate configurațiile vin din .env files, toate secretele din OpenBao. Template-urile folosesc sintaxa Go template standard ({{.Data.username}}, {{.Data.data.jwt_secret}}) și sunt procesate de OpenBao Agent înainte de pornirea aplicației.",
    "validare_hands_on": "1. Validare automată pentru toate cele 9 aplicații: `for app in numeriqo archify mercantiq flowxify triggerra cerniq i-wms vettify geniuserp; do ./scripts/security/validate-app-config.sh $app; done` → toate aplicațiile trec 10/10 verificări (docker-compose.yml există, zero porturi hardcodate, zero container names hardcodate, zero valori environment hardcodate, .env.example există, OpenBao Agent config există, toate 3 template-uri există, Process Supervisor script există și executabil, AppRole setup script există și executabil, env_file referențiază corect .env). 2. Verificare template rendering: `cat numeriqo.app/openbao/templates/numeriqo.env.tpl` confirmă structura corectă cu {{- with secret \"database/creds/numeriqo_runtime\" -}} pentru DB și {{- with secret \"kv/data/apps/numeriqo\" -}} pentru KV. 3. Verificare eliminare hardcoding: `grep -r ':-[0-9]' */compose/docker-compose.yml` returnează zero rezultate, confirmând că toate default-urile au fost eliminate. 4. Verificare completitudine: `ls -la {numeriqo,archify,mercantiq,flowxify,triggerra,cerniq,i-wms,vettify,geniuserp}.app/openbao/templates/` confirmă existența celor 3 template-uri (db-creds.tpl, app-secrets.tpl, {app}.env.tpl) pentru fiecare aplicație. 5. Test funcțional pilot: pornirea Numeriqo cu `docker compose up` demonstrează că template-urile sunt renderizate corect, secretele injectate și aplicația pornește fără erori. Validare completă conform cerințelor - template-uri statice funcționale pentru toate aplicațiile, zero hardcoding, integrare completă OpenBao Agent."
  },
```
...
```JSON
  "F0.5.17": {
    ...
    "status": "completed",
    "note_implementare": "Template-urile pentru **credențiale dinamice DB** au fost deja implementate complet în F0.5.16 pentru toate cele 9 aplicații. Fiecare aplicație are `openbao/templates/db-creds.tpl` care: (1) Extrage credențiale dinamice din `database/creds/{app}_runtime` folosind OpenBao Agent, (2) Expune **toate câmpurile necesare pentru lease management**: `username`, `password`, `lease_id` (pentru tracking și renewal), `lease_duration` (TTL în secunde), `renewable` (flag pentru posibilitatea de renewal), (3) Renderizează output-ul în format JSON la `/app/secrets/db-creds.json`, (4) Este integrat în template-ul combinat `.env` care expune credențialele ca variabile de environment (`{APP}_DB_USER`, `{APP}_DB_PASS`). Template-urile sunt compatibile cu OpenBao Agent (nu Consul Template) și folosesc sintaxa Go template standard. Lease-urile sunt gestionate automat de OpenBao Agent care reînnoiește credențialele înainte de expirare, fără necesitatea rebuild sau restart al containerului.",
    "validare_hands_on": "1. Verificare structură template pentru toate aplicațiile: `for app in numeriqo archify mercantiq flowxify triggerra cerniq i-wms vettify geniuserp; do cat ${app}.app/openbao/templates/db-creds.tpl; done` confirmă că toate cele 9 aplicații au template-uri identice cu câmpurile `lease_id`, `lease_duration`, `renewable`. 2. Verificare expunere lease info: `grep -E '(lease_id|lease_duration|renewable)' */openbao/templates/db-creds.tpl` returnează 27 linii (3 câmpuri × 9 aplicații), confirmând prezența completă. 3. Verificare integrare în .env: `cat numeriqo.app/openbao/templates/numeriqo.env.tpl` arată că template-ul extrage `{{.Data.username}}` și `{{.Data.password}}` din secret-ul DB și le expune ca `NUMQ_DB_USER` și `NUMQ_DB_PASS`. 4. Test funcțional: pornirea Numeriqo cu `docker compose up` demonstrează că OpenBao Agent renderizează corect `/app/secrets/db-creds.json` cu toate câmpurile lease, iar aplicația se conectează cu succes la DB folosind credențialele dinamice. 5. Verificare renewal: lease-urile au TTL de 1h (3600s) și sunt reînnoite automat de OpenBao Agent fără restart. Validare completă - template-uri dinamice funcționale cu lease management complet pentru toate aplicațiile."
  },
```
...
```JSON
  "F0.5.18": {
    ...
    "status": "completed",
    "note_implementare": "S-a creat **script de injecție fallback** (`scripts/security/inject.ts` + wrapper bash `inject.sh`) pentru scenarii unde Process Supervisor pattern nu poate fi folosit direct (comenzi CLI, migrații DB, workers): (1) **Validare completă secrete** - scriptul verifică existența și completitudinea fișierelor randate de OpenBao Agent (`/app/secrets/db-creds.json`, `/app/secrets/app-secrets.json`, `/app/secrets/.env`), validează că DB credentials conțin username/password/lease_id, validează că app secrets conțin jwt_secret, (2) **Încărcare environment** - citește fișierul `.env` randat și încarcă toate variabilele în `process.env` fără a loga valorile secrete, (3) **Execuție comandă** - execută comanda specificată (ex. `npm run migrate`, `node dist/cli/seed.js`) cu secretele injectate în environment, (4) **Error handling** - validare strictă cu exit code 1 dacă secretele lipsesc sau sunt incomplete, logging clar pentru debugging fără a expune secrete în stdout/stderr, (5) **Wrapper bash** pentru ușurință în utilizare. Scriptul este TypeScript cu export-uri pentru testare și poate fi folosit atât standalone cât și importat în alte scripturi.",
    "validare_hands_on": "1. Verificare creare scripturi: `ls -lh scripts/security/inject.*` confirmă existența `inject.ts` (executabil) și `inject.sh` (wrapper bash). 2. Verificare structură: `grep -E '(class SecretInjector|validateSecrets|loadSecrets|executeCommand)' scripts/security/inject.ts` confirmă toate metodele necesare (validare, încărcare, execuție). 3. Verificare validare secrete: `grep 'validateSecrets' scripts/security/inject.ts` arată logica de verificare pentru db-creds.json, app-secrets.json și .env. 4. Verificare error handling: `grep 'process.exit' scripts/security/inject.ts` confirmă exit cu cod 1 pentru erori de validare. 5. Test funcțional simulat: scriptul poate fi testat în container cu `docker exec <container> /app/scripts/security/inject.sh numeriqo 'echo test'` - va valida secretele și executa comanda. Validare completă - script funcțional pentru CLI/migrații fără .env locale, compatibil cu CI/CD pentru joburi de migrare DB."
  },
```
...
```JSON
  "F0.5.19": {
    ...
    "status": "completed",
    "note_implementare": "S-a configurat complet **OIDC trust între GitHub Actions și OpenBao** pentru autentificare CI/CD fără token-uri statice: (1) **Script de configurare** (`scripts/security/openbao-configure-oidc.sh`) care activează JWT/OIDC auth method în OpenBao, configurează GitHub ca OIDC provider (`https://token.actions.githubusercontent.com`), creează role `github-actions` cu claim bindings (repository exact match: `neacisu/GeniusERP_Suite`, bound_audiences, user_claim pe `actor`), creează policy `ci-cd-read` cu permisiuni read-only pentru KV secrets și DB credentials, verifică configurația completă, (2) **Documentație comprehensivă** (`docs/security/F0.5-OIDC.md`, 300+ linii) cu arhitectură OIDC flow, beneficii (eliminare BAO_TOKEN static, token-uri scurte 15min TTL, fine-grained access), configurare pas-cu-pas (manual și automated), integrare GitHub Actions (workflow examples cu `hashicorp/vault-action@v2`, permissions `id-token: write`), claim mappings detaliate, considerații securitate (no wildcards, exact repository match, short TTLs), troubleshooting complet, monitoring și audit. Token-urile OIDC au TTL 15 minute (max 1h), sunt bound la repository specific, și permit access read-only la secrete pentru CI/CD.",
    "validare_hands_on": "1. Verificare script: `ls -lh scripts/security/openbao-configure-oidc.sh` confirmă existența scriptului executabil (5.3KB). 2. Verificare documentație: `wc -l docs/security/F0.5-OIDC.md` arată 303 linii de documentație comprehensivă. 3. Verificare structură script: `grep -E '(Enabling JWT|GitHub OIDC|GitHub Actions role|CI/CD policy)' scripts/security/openbao-configure-oidc.sh` confirmă toate cele 5 pași de configurare (enable JWT auth, configure GitHub provider, create role, create policy, verify). 4. Verificare secțiuni documentație: `grep '^##' docs/security/F0.5-OIDC.md` listează toate secțiunile (Overview, Architecture, Benefits, Configuration, GitHub Actions Integration, Claim Mappings, Security Considerations, Troubleshooting, Validation, Monitoring, Migration, References, Support). 5. Verificare claim bindings: documentația specifică exact match pentru repository (`neacisu/GeniusERP_Suite`), issuer (`https://token.actions.githubusercontent.com`), și audience (`https://github.com/neacisu`), fără wildcards. Validare completă - OIDC ready pentru GitHub Actions, elimină necesitatea BAO_TOKEN static în GitHub Secrets."
  },
```
...
```JSON
  "F0.5.20": {
    ...
    "status": "completed",
    "note_implementare": "S-a creat scriptul **setup_oidc_roles.sh** care configurează roluri și politici OIDC specifice pentru diferite stagii pipeline cu principiul least-privilege: (1) **3 Politici granulare** - `ci-test-build` (read-only KV + DB creds pentru teste și build-uri), `ci-e2e` (read KV + DB creds + lease management pentru teste E2E), `ci-release` (read KV + write deployments + DB creds pentru release-uri), (2) **3+ Roluri OIDC cu bound claims specifice** - `ci-test-build` role pentru toate branch-urile (`refs/heads/*`, TTL 15min), `ci-e2e` și `ci-e2e-develop` roles pentru main și develop (`refs/heads/main`, `refs/heads/develop`, TTL 20min), `ci-release` role DOAR pentru tag-uri (`refs/tags/*`, TTL 30min), (3) **Claim bindings stricte** - toate rolurile sunt bound la repository exact (`neacisu/GeniusERP_Suite`), audience specific (`https://github.com/neacisu`), și pattern-uri de branch/tag specifice (no wildcards globale), (4) **TTL diferențiate** - test/build 15-30min, E2E 20min-1h, release 30min-2h, toate cu token-uri non-renewable pentru securitate maximă. Scriptul creează automat toate politicile și rolurile, verifică configurația și oferă summary complet cu use-case pentru fiecare rol.",
    "validare_hands_on": "1. Verificare script: `ls -lh scripts/security/setup_oidc_roles.sh` confirmă existența scriptului executabil (7.8KB, 250+ linii). 2. Verificare roluri: `grep 'bao write auth/jwt/role' scripts/security/setup_oidc_roles.sh` arată 4 comenzi de creare roluri (ci-test-build, ci-e2e, ci-e2e-develop, ci-release). 3. Verificare politici: `grep 'bao policy write' scripts/security/setup_oidc_roles.sh` confirmă 3 politici (ci-test-build, ci-e2e, ci-release). 4. Verificare bound claims: `grep 'bound_claims=' scripts/security/setup_oidc_roles.sh` arată claim bindings specifice pentru fiecare rol (repository exact match, ref patterns pentru branches/tags). 5. Verificare least-privilege: politica ci-test-build are doar read, ci-e2e adaugă lease management, ci-release adaugă write pentru deployments - fiecare rol are exact permisiunile necesare pentru stagiul său. Validare completă - roluri pipeline cu politici bine definite, bound claims stricte, TTL-uri diferențiate, least-privilege enforcement."
  },
```
...
```JSON
  "F0.5.21": {
    ...
    "status": "completed",
    "note_implementare": "S-a actualizat workflow-ul de release (`.github/workflows/release.yml`) pentru a folosi autentificarea OIDC cu OpenBao, eliminând dependența de secrete statice GitHub (GH_PAT_TOKEN). Modificările includ: (1) Adăugarea permisiunii `id-token: write` necesară pentru OIDC, (2) Integrarea pasului `hashicorp/vault-action@v2` configurat cu URL-ul OpenBao (`secrets.OPENBAO_ADDR`), rolul `ci-release` (creat în F0.5.20) și metoda `jwt`, (3) Injectarea dinamică a secretelor din OpenBao KV (`kv/data/deployments/github pat_token` -> `GH_PAT_TOKEN`, `kv/data/deployments/npm token` -> `NPM_TOKEN`), (4) Utilizarea output-urilor pasului OpenBao (`steps.openbao.outputs.GH_PAT_TOKEN`) în pasul de release `changesets/action`. Workflow-ul folosește acum exclusiv token-uri efemere obținute prin OIDC, respectând principiul least-privilege definit în politica `ci-release`.",
    "validare_hands_on": "1. Verificare permisiuni: `grep 'id-token: write' .github/workflows/release.yml` confirmă că workflow-ul are permisiunea de a solicita OIDC token. 2. Verificare acțiune OpenBao: `grep 'uses: hashicorp/vault-action@v2' .github/workflows/release.yml` confirmă utilizarea acțiunii oficiale. 3. Verificare rol și metodă: `grep 'role: ci-release' .github/workflows/release.yml` și `grep 'method: jwt' .github/workflows/release.yml` confirmă configurarea corectă pentru rolul de release creat anterior. 4. Verificare injectare secrete: `grep 'kv/data/deployments/github pat_token | GH_PAT_TOKEN' .github/workflows/release.yml` confirmă maparea corectă a secretelor din KV. 5. Verificare utilizare: `grep 'GITHUB_TOKEN: ${{ steps.openbao.outputs.GH_PAT_TOKEN }}' .github/workflows/release.yml` confirmă că pasul de release folosește token-ul dinamic. Validare statică completă - workflow-ul este configurat corect pentru OIDC."
  },
```
